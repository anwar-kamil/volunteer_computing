...
cout<<"\nSending following Cpu Info to Server: \n"<<cpuinfo;
if (send(sock, cpuinfo, 100, 0) == -1)
perror("send");

if ((bytes = recv(sock, buffer, sizeof (buffer), 0)) == -1) 
perror("recv");
printf ("\nAlgorithm File recieving from Server: %s\n", buffer);

char * buffer1 = new char [100000];
if ((bytes = recv(sock, buffer1, 100000, 0)) == -1) 
perror("recv");
write_file(buffer,buffer1);

cout<<"\nPlease wait while given algorithm executes...";
cout<<"\nAlgorithm executed in : "<<algo_run(buffer)<<" seconds.\n";

strcpy (buffer, "output.txt");
cout<<"\nsending Output File to server: "<<buffer<<endl<<endl;
if (send(sock, buffer, 100, 0) == -1)
perror("send");
strcpy(buffer1,read_file(buffer));
if (send(sock, buffer1, 100000, 0) == -1)
perror("send");
close(sock);
return 0;
}
...
size_t s=100; size_t * s1=new size_t[1];
char * buffer = new char [100];
while(1)
{
...
if(t=='n')
	while (((childpid = waitpid(p,&status,0)) == -1) && (errno == EINTR)) ;
else if(t=='h')
